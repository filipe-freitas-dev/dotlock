...
impl DotLockError {
    pub fn hint(&self) -> Option<&'static str> {
        match self {
            ...
            DotLockError::WeakPassword { .. } => Some(
                "use at least 12 characters with lowercase, uppercase, a digit and a symbol",
            ),
            ...
            DotLockError::InvalidVariableName { .. } => Some(
                "use only letters, digits and underscores; the name cannot start with a digit",
            ),
            DotLockError::EnvParseError { .. } => {
                Some("each line must look like `KEY=VALUE` (quotes and `export` prefix are allowed)")
            }
