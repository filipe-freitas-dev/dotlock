...
use crate::{
    ...
    storage::identity::{LocalIdentityMetadata, load_local_identity_metadata, public_key_path},
};
...
pub struct VerifyContext<'a> {
    ...
    pub strict: bool,
    pub local_identity: Option<&'a LocalIdentityMetadata>,
    pub local_public_key: Option<&'a str>,
...
pub fn verify_entries(entries: &[AuditEntry], ctx: &VerifyContext<'_>) -> DotLockResult<usize> {
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        } else if let (Some(identity), Some(public_key)) =
            (ctx.local_identity, ctx.local_public_key)
        {
...
fn verify_high_water_mark(entries: &[AuditEntry], ctx: &VerifyContext<'_>) -> DotLockResult<()> {
    ...
    let (Some(identity), Some(public_key)) = (ctx.local_identity, ctx.local_public_key) else {
        return Err(DotLockError::Crypto(
            "audit verify failed: signer identity is unavailable for the high-water mark"
                .to_string(),
        ));
    };
    if identity.fingerprint != hwm.signer_fingerprint {
        return Err(DotLockError::Crypto(
...
pub fn verify_log(strict: bool) -> DotLockResult<()> {
    ...
    let local_public_key = public_key_path()
        .ok()
    ...
    let ctx = VerifyContext {
        strict,
        local_identity: local_identity.as_ref(),
        local_public_key: local_public_key.as_deref(),
        hwm: hwm.as_ref(),
    };
...
mod tests {
    ...
    use super::{VerifyContext, verify_entries};
    use crate::{
        ...
        storage::identity::LocalIdentityMetadata,
    };
    ...
    fn ctx<'a>(
        strict: bool,
        meta: &'a LocalIdentityMetadata,
        public_key: &'a str,
        hwm: Option<&'a AuditHighWaterMark>,
    ) -> VerifyContext<'a> {
        VerifyContext {
            strict,
            local_identity: Some(meta),
            local_public_key: Some(public_key),
            hwm,
        }
    }

    fn meta_for(identity: &GeneratedIdentity) -> LocalIdentityMetadata {
    ...
    fn signed_chain_with_matching_hwm_verifies_in_strict_mode() {
        ...
        let meta = meta_for(&identity);

        let anonymous = verify_entries(
            &entries,
            &ctx(true, &meta, &identity.public_key_pem, Some(&hwm)),
        )
        .expect("verify");
    ...
    fn anonymous_entry_fails_strict_verify_but_passes_lax() {
        ...
        let strict = verify_entries(&entries, &ctx(true, &meta, &identity.public_key_pem, None));
        assert!(
            strict.is_err(),
        ...
        let anonymous =
            verify_entries(&entries, &ctx(false, &meta, &identity.public_key_pem, None))
                .expect("lax verify");
    ...
    fn tail_truncation_is_detected_even_in_lax_mode() {
        ...
        for strict in [true, false] {
            let result = verify_entries(
                truncated,
                &ctx(strict, &meta, &identity.public_key_pem, Some(&hwm)),
            );
            assert!(
                result.is_err(),
    ...
    fn hwm_head_hash_mismatch_is_rejected() {
        ...
        let result = verify_entries(
            &entries,
            &ctx(false, &meta, &identity.public_key_pem, Some(&hwm)),
        );
